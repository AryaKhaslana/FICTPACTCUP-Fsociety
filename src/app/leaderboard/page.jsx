...
export default async function LeaderboardPage() {
  ...
  // 2. TARIK DATA DARI DATABASE (TAMBAHIN avatarUrl)
  ...
  // 👇 6. BIKIN VARIABEL BUAT DILEMPAR KE NAVBAR 👇
  // (Bang Sepuh cari data user yg login dari sortedUsers, kalo ga ada pake data default)
  const loggedInUserFromList = sortedUsers.find(u => u.id === loggedInUserId);
  const namaSiswa = loggedInUserFromList?.name || "Kim Booyah";
  const avatarSiswa = loggedInUserFromList?.avatarUrl || null;

  return (
    <div className="min-h-screen bg-[#000010] text-white font-poppins pb-32">
      
      {/* 👇 7. OPER AVATAR KE NAVBAR JUGA BIAR MATCH! 👇 */}
      ...
      <main className="max-w-5xl mx-auto px-4 md:px-6 pt-12 flex flex-col items-center">
        ...
        {/* JUDUL HALL OF FAME */}
        <h1 className="text-4xl md:text-5xl font-pixel font-black text-white mb-10 mt-12 tracking-widest uppercase drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">
          Hall of fame
        </h1>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-20 md:mb-28">
          ...
          <button className="bg-[#F59E0B] text-[#000010] px-6 py-2 rounded-xl font-black shadow-[0_0_20px_rgba(245,158,11,0.5)] transition-transform hover:scale-105">
            Sepanjang masa
          </button>
        </div>

        {/* PODIUM JUARA MENGGUNAKAN DATA DINAMIS DARI DATABASE */}
        <div className="flex justify-center items-end gap-2 md:gap-6 w-full max-w-3xl px-2 relative z-20">
          ...
          <div className="flex flex-col items-center w-1/3">
            <div className="relative z-10 -mb-10 md:-mb-14 transition-transform hover:-translate-y-2">
              <div className="w-20 h-20 md:w-28 md:h-28 rounded-full overflow-hidden border-4 border-[#B45309] bg-gray-800 shadow-[0_0_20px_rgba(217,119,6,0.4)]">
                {/* 🔥 UBAH PAKE AVATAR URL 🔥 */}
                <img 
                  src={podiumUsers[1]?.avatarUrl || `https://api.dicebear.com/7.x/adventurer/svg?seed=${podiumUsers[1]?.name || 'Kosong'}`} 
                  alt="Juara 2" 
            ...
            <div className="w-full bg-gradient-to-t from-[#B45309] to-[#D97706] h-48 md:h-60 rounded-t-2xl pt-12 md:pt-16 pb-4 px-1 flex flex-col items-center justify-start text-center shadow-2xl relative overflow-hidden border-t-2 border-[#F59E0B]">
              ...
              <p className="text-[10px] md:text-xs text-orange-200 font-semibold mb-3 line-clamp-1">{podiumUsers[1]?.role || "-"}</p>
          ...
          <div className="flex flex-col items-center w-1/3 z-20">
            <div className="relative z-10 -mb-12 md:-mb-16 flex flex-col items-center transition-transform hover:-translate-y-2">
              ...
              <div className="w-24 h-24 md:w-36 md:h-36 rounded-full overflow-hidden border-[6px] border-yellow-400 bg-gray-800 shadow-[0_0_30px_rgba(250,204,21,0.6)]">
                {/* 🔥 UDAH GAK HARDCODE GOJO LAGI! 🔥 */}
                <img 
                  src={podiumUsers[0]?.avatarUrl || `https://api.dicebear.com/7.x/adventurer/svg?seed=${podiumUsers[0]?.name || 'Kosong'}`} 
                  alt="Juara 1" 
                  className="w-full h-full object-cover" 
                />
              </div>
            ...
            <div className="w-full bg-gradient-to-t from-[#D97706] to-[#F59E0B] h-64 md:h-80 rounded-t-2xl pt-16 md:pt-20 pb-4 px-1 flex flex-col items-center justify-start text-center shadow-[0_0_40px_rgba(245,158,11,0.3)] relative overflow-hidden border-t-4 border-yellow-400">
              ...
              <p className="text-[10px] md:text-xs text-[#FFFFFF]/80 font-bold mb-3 line-clamp-1">{podiumUsers[0]?.role || "-"}</p>
          ...
          <div className="flex flex-col items-center w-1/3">
            <div className="relative z-10 -mb-10 md:-mb-14 transition-transform hover:-translate-y-2">
              <div className="w-20 h-20 md:w-28 md:h-28 rounded-full overflow-hidden border-4 border-[#92400E] bg-gray-800 shadow-[0_0_20px_rgba(146,64,14,0.5)]">
                 {/* 🔥 UBAH PAKE AVATAR URL 🔥 */}
                <img 
                  src={podiumUsers[2]?.avatarUrl || `https://api.dicebear.com/7.x/adventurer/svg?seed=${podiumUsers[2]?.name || 'Kosong'}`} 
                  alt="Juara 3" 
                  className="w-full h-full object-cover" 
                />
              </div>
            ...
            <div className="w-full bg-gradient-to-t from-[#78350F] to-[#92400E] h-40 md:h-52 rounded-t-2xl pt-12 md:pt-16 pb-4 px-1 flex flex-col items-center justify-start text-center shadow-xl relative overflow-hidden border-t-2 border-[#D97706]">
              ...
              <p className="text-[10px] md:text-xs text-orange-200 font-semibold mb-3 line-clamp-1">{podiumUsers[2]?.role || "-"}</p>
        ...
        {/* z-0 biar kotaknya di belakang efek bayangan podium */}
